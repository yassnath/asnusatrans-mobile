...
({
  ...
  final sourceItems = items.map((item) => Map<String, dynamic>.from(item)).toList();
  ...
  final firstDate = mergedDetails
      .map((row) => Formatters.parseDate(row['armada_start_date'] ?? row['tanggal']))
      .whereType<DateTime>()
      .fold<DateTime?>(
        null,
  ...
  _FixedInvoiceBatch? fixedBatch,
    ...
    try {
      final invoiceDetailList =
          detailList.isNotEmpty ? detailList : _dashboardToDetailList(item['rincian']);
      ...
      pw.Widget buildInvoiceContent({
        ...
        final incomeColumnWidths = _buildDashboardIncomeTableColumnWidths(printableRows);
