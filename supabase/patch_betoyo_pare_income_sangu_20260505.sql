...
where pg_temp.cvant_patch_norm(customer_name) = ''
  ...
  and pg_temp.cvant_patch_norm(lokasi_bongkar) = 'pare';
...
values
  (null, 'Betoyo', 'Pare', 87.00, null, 125, true);

delete from public.sangu_driver_rules
where pg_temp.cvant_patch_norm(lokasi_muat) = 'betoyo'
  and (
    pg_temp.cvant_patch_norm(tempat) = 'betoyo pare'
    or pg_temp.cvant_patch_norm(lokasi_bongkar) = 'pare'
  );
...
values
  ('BETOYO - PARE', 'BETOYO', 'PARE', 1165000.00, true);
