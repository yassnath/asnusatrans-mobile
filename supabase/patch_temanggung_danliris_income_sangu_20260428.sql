...
where pg_temp.cvant_patch_norm(customer_name) = ''
  and pg_temp.cvant_patch_norm(lokasi_muat) = ''
  and pg_temp.cvant_patch_norm(lokasi_bongkar) in ('temanggung', 'danliris');
...
where pg_temp.cvant_patch_norm(lokasi_muat) = ''
  and (
    pg_temp.cvant_patch_norm(tempat) = 'temanggung'
    or pg_temp.cvant_patch_norm(lokasi_bongkar) = 'temanggung'
  );
...
values
  ('TEMANGGUNG', null, 'TEMANGGUNG', 2435000.00, true);
