...
bool PrintJob::printPdf(const std::string& name,
  ...
  if (!printer.empty()) {
    const auto driverExtra =
        DeviceCapabilities(fromUtf8(printer).c_str(), NULL, DC_EXTRA, NULL,
                           NULL);
    if (driverExtra > 0) {
      dmExtra = static_cast<std::size_t>(driverExtra);
    }
  }

  HGLOBAL dmHandle = nullptr;

  if (!usePrinterSettings) {
    dmHandle = GlobalAlloc(GMEM_MOVEABLE | GMEM_ZEROINIT, dmSize + dmExtra);
    if (!dmHandle) {
      return false;
    }

    auto dm = static_cast<DEVMODE*>(GlobalLock(dmHandle));
    if (!dm) {
      GlobalFree(dmHandle);
      return false;
    }
  ...
  if (printer.empty()) {
    ...
    DEVMODE* dm = nullptr;
    if (dmHandle != nullptr) {
      dm = static_cast<DEVMODE*>(GlobalLock(dmHandle));
    }
...
void PrintJob::writeJob(std::vector<uint8_t> data) {
  std::lock_guard<std::mutex> pdfium_lock(g_pdfium_mutex);
  ...
  if (r <= 0) {
    disposeHandles();
    printing->onCompleted(this, false, "Cannot start print document");
    return;
  }
  ...
  if (!doc) {
    ...
    FPDF_DestroyLibrary();
    disposeHandles();
    printing->onCompleted(this, false, "Cannot load PDF document for printing");
    return;
  }
