  ...
  HRESULT __stdcall Activate(
    ...
    try {
      ...
      for (ULONG i = 0; i < count; i++) {
        auto item = data[i];
        const std::string key = wstring_to_utf8(item.Key != nullptr ? item.Key : L"");
        const std::string value = wstring_to_utf8(item.Value != nullptr ? item.Value : L"");
      ...
      const auto payload = wstring_to_utf8(args != nullptr ? args : L"");
